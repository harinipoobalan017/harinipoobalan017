<h1 align="center">Hi 👋, I'm Harini P</h1>

<h3 align="center">
IoT & Cybersecurity Engineer • Embedded Systems • AI/ML • Full Stack Development
</h3>

<p align="center">
Building secure, intelligent and scalable real-world systems with IoT, AI and modern software technologies.
</p>

<p align="center">
<img src="https://readme-typing-svg.demolab.com?font=Poppins&weight=600&size=22&pause=1000&color=8A2BE2&center=true&vCenter=true&width=750&lines=IoT+Developer;Cybersecurity+Enthusiast;Embedded+Systems+Engineer;AI%2FML+Explorer;Blockchain+Developer;Full+Stack+Developer;Always+Building+%26+Learning" alt="Typing SVG" />
</p>

<p align="center">
<img src="https://komarev.com/ghpvc/?username=harinipoobalan017&label=Profile+Views&color=8A2BE2&style=for-the-badge" />
</p>

---

## 👩‍💻 About Me

🎓 **Bachelor of Engineering** in **IoT & Cybersecurity including Blockchain Technology**

🏫 **Dayananda Sagar College of Engineering (DSCE), Bangalore**

🎓 **Diploma in Computer Science** — Ramaiah Polytechnic

💼 **Former Full Stack Development Intern** — Integra Micro Systems

I am passionate about building technology that connects the physical and digital worlds. My interests span **IoT, embedded systems, AI/ML, cybersecurity, blockchain and full-stack development**, with a focus on developing practical solutions for real-world problems.

---

## 🚀 Areas of Interest

* 🌐 Internet of Things & Smart Systems
* ⚙️ Embedded Systems & Edge Computing
* 🤖 Artificial Intelligence & Machine Learning
* 🔐 Cybersecurity & Secure Systems
* 🔗 Blockchain & Decentralized Technologies
* 💻 Full Stack Web Development
* ☁️ Cloud Computing
* 📡 Real-Time Automation
* 🧠 AI-Powered Applications

---

# 💻 Technical Skills

### Programming Languages

<p>
<img src="https://skillicons.dev/icons?i=python" />
<img src="https://skillicons.dev/icons?i=java" />
<img src="https://skillicons.dev/icons?i=javascript" />
<img src="https://skillicons.dev/icons?i=typescript" />
<img src="https://skillicons.dev/icons?i=c" />
<img src="https://skillicons.dev/icons?i=cpp" />
</p>

### Web Development

<p>
<img src="https://skillicons.dev/icons?i=react" />
<img src="https://skillicons.dev/icons?i=nodejs" />
<img src="https://skillicons.dev/icons?i=express" />
<img src="https://skillicons.dev/icons?i=html" />
<img src="https://skillicons.dev/icons?i=css" />
<img src="https://skillicons.dev/icons?i=tailwind" />
</p>

### Databases

<p>
<img src="https://skillicons.dev/icons?i=mongodb" />
<img src="https://skillicons.dev/icons?i=mysql" />
</p>

### Embedded Systems & IoT

<p>
<img src="https://skillicons.dev/icons?i=arduino" />
<img src="https://skillicons.dev/icons?i=raspberrypi" />
<img src="https://img.shields.io/badge/ESP32-E7352C?style=for-the-badge&logo=espressif&logoColor=white" />
</p>

### Tools & Platforms

<p>
<img src="https://skillicons.dev/icons?i=git" />
<img src="https://skillicons.dev/icons?i=github" />
<img src="https://skillicons.dev/icons?i=docker" />
<img src="https://skillicons.dev/icons?i=linux" />
<img src="https://skillicons.dev/icons?i=vscode" />
<img src="https://skillicons.dev/icons?i=postman" />
</p>

---

# 🚀 Featured Projects

### 🛡️ QuantumVault-X

AI-powered decentralized cybersecurity vault combining **AES-256 encryption, blockchain verification and secure data management**.

**Focus:** Cybersecurity • AI • Blockchain • Secure Storage

---

### 🚦 Smart Traffic Routing System

An intelligent IoT-based traffic management system designed to optimize traffic flow through **real-time monitoring, intelligent routing and congestion prediction**.

**Focus:** IoT • AI/ML • Smart Cities • Real-Time Systems

---

### 🤖 Rakshak

AI-powered emergency management and smart protection platform featuring **real-time monitoring, analytics dashboards and automated response mechanisms**.

**Focus:** AI • IoT • Automation • Emergency Management

---

### 🔒 SentinelGPT-X

AI-powered **SOC/XDR platform** designed for cybersecurity monitoring, threat intelligence, anomaly detection and incident response.

**Focus:** AI • Cybersecurity • SOC • Threat Intelligence

---

### 🌐 Blockchain-Secured IoT Communication

A blockchain-enabled security framework for IoT communication that focuses on **device authentication, data integrity and trusted communication**.

**Focus:** Blockchain • IoT • Cybersecurity • Distributed Systems

---

# 📊 GitHub Analytics

<p align="center">
<img width="49%" src="https://github-readme-stats.vercel.app/api?username=harinipoobalan017&show_icons=true&theme=github_dark&hide_border=true" />

<img width="49%" src="https://github-readme-stats.vercel.app/api/top-langs/?username=harinipoobalan017&layout=compact&theme=github_dark&hide_border=true" />
</p>

---

# 🔥 GitHub Streak

<p align="center">
<img src="https://streak-stats.demolab.com?user=harinipoobalan017&theme=tokyonight&hide_border=true" />
</p>

---

# 🏅 Certifications & Programs

### 🐧 Linux Foundation

<p align="center">

<a href="https://www.credly.com/badges/158fca11-6091-471f-a128-640b93da318a" target="_blank">

<img
src="https://images.credly.com/size/340x340/images/158fca11-6091-471f-a128-640b93da318a.png"
width="180"
alt="Linux Foundation - Migrating AI Workflows Across Platforms"
/>

</a>

</p>

<p align="center">

<b>Migrating AI Workflows Across Platforms</b><br>

Linux Foundation • <code>LFEL1008</code>

</p>

<p align="center">

<a href="https://www.credly.com/badges/158fca11-6091-471f-a128-640b93da318a" target="_blank">
View Credential →
</a>

</p>

---

### 📜 Other Learning Programs

| Organization                    | Area                          |
| ------------------------------- | ----------------------------- |
| 🐧 **Linux Foundation**         | AI & Open Source Technologies |
| 🌐 **Cisco Networking Academy** | Networking & Technology       |
| 💻 **IBM SkillsBuild**          | AI & Emerging Technologies    |
| ☁️ **AWS Skill Builder**        | Cloud & AWS Technologies      |

> More certifications and credentials will be added as I continue learning and completing professional programs.

---

# 🎯 Current Focus

* 🚀 Building secure AI-powered applications
* 🤖 Developing embedded & IoT systems
* 🧠 Exploring Edge AI and intelligent devices
* 🔐 Strengthening cybersecurity skills
* ☁️ Learning cloud and distributed systems
* 🌍 Contributing to open-source projects
* 📚 Continuously improving through hands-on projects

---

# 📈 Learning Journey

```text
IoT & Embedded Systems
        ↓
AI / Machine Learning
        ↓
Edge AI & Intelligent Devices
        ↓
Cybersecurity & Secure Systems
        ↓
Cloud & Distributed Computing
        ↓
Research • Open Source • Real-World Solutions
```

---

# 🌐 Connect With Me

<p align="center">

<a href="YOUR_LINKEDIN">
<img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>

<a href="mailto:YOUR_EMAIL">
<img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white" />
</a>

<a href="https://github.com/harinipoobalan017">
<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>

<a href="YOUR_PORTFOLIO">
<img src="https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white" />
</a>

</p>

---

# 💡 Philosophy

<h3 align="center">

"Engineering Secure, Intelligent & Future-Ready Technologies."

</h3>

<p align="center">
Building • Learning • Researching • Innovating 🚀
</p>

---

<p align="center">

⭐ <b>Thanks for visiting my profile!</b> ⭐

<br>

If you find my projects interesting, feel free to explore my repositories and leave a ⭐

</p>
